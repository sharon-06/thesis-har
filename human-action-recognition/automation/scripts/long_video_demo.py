...
FONTSCALE = 1
FONTCOLOR = (128, 128, 128)  # BGR, gray
...
def show_results_video(model, data, label, args):
...
def show_results_json(model, data, label, args):
    frame_queue = deque(maxlen=args.sample_length)
    result_queue = deque(maxlen=1)
...
def main():
    ...
    show_results = show_results_json if args.out_file.endswith(
        '.json') else show_results_video
